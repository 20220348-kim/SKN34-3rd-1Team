...
function messageFor(code: string, status: number): string {
  ...
  if (code === 'APPLICATION_PREPARATION_REVISION_CONFLICT') return '입력 또는 작성본이 변경되었습니다. 최신 내용을 다시 불러와 확인해 주세요.'
  ...
  if (code === 'APPLICATION_DOCUMENT_MAPPING_FAILED') return '일부 답변의 기입 위치를 확인하지 못해 파일을 생성하지 않았습니다. 공식 양식과 답변을 확인해 주세요.'
