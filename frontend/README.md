...
관심 공고함은 `/api/v1/me/saved-programs`를 쓰며 공고 상세의 관심 공고 저장 버튼(비로그인은 로그인 링크)과 모집글 상세의 `관심 공고에 추가` 버튼이 담습니다. 두 버튼은 `presentation/shared/support-program/useSupportProgramSaveViewModel`을 함께 씁니다. 진행 관리는 기존 신청 준비 목록 API를 재사용하며, 각 건의 독립된 진행 revision으로 단계 변경을 저장합니다. 로그인 작업 사이드바는 흰색 배경과 둥근 메뉴 항목을 사용하며,
...
이동해도 끊기지 않고, 결과는 Redux로 돌아옵니다. 화면 밖에 있는 동안은 사이드바 대화 기록 아래(스크롤 영역 밖, 계정 줄 위)에 고정된
진행 패널(`shared/chat-activity/ChatActivityPanel`)이 점·상태(검색 중, 결과 N건 도착, 조건 변경안 준비됨, 실패 등)·대화 제목·"보기" 링크를 보여 주고,
검색 중인 대화의 기록 항목에도 같은 점이 붙습니다. 사이드바를 접었거나 모바일·공개 헤더에서는 헤더 칩이 같은 상태를 알립니다.
결과가 도착하면 오른쪽 위 헤더 아래에 한 줄 알림(12초 뒤 자동 접힘, "대화 보기"로 이동)을 띄웁니다. 몇 초면 끝나는 조건 해석은
패널·헤더 칩에 진행 중 표시 없이 결과가 도착했을 때만 알리고(기록 항목의 점은 해석 중에도 붙음), 자유 질문 답변("답변이 도착했어요")은 알림을 끄고 패널·배지로만 알립니다.
채팅 화면이 열리면 `outcomeSeen`으로 패널·점·알림을 지웁니다. 비로그인 대화는 검색 흐름을 떠나면 비우지만, 진행 중이거나 아직 보지 않은
결과가 있는 동안은 지우지 않습니다. 요청 취소는 로그아웃·취소 버튼·90초 시간 초과, 그리고 진행 중에 검색 화면에서 사이드바의 새검색을 다시 누르거나 다른 대화 기록을
