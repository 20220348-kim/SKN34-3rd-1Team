...
로컬 데모 적재는 V35의 nullable `demo_seed_key`로 목업과 일반 신청 작업을 구분합니다. 계정별 목업 키만 유일하며
일반 API가 만드는 NULL 키의 작업은 계속 추가할 수 있습니다. 공개 API 계약은 같고, 자동 적재 흐름은 [인프라 데모 데이터](../../infrastructure/README.md#데모-데이터)를 따릅니다.
